...
from .models import User, SolvedTask
...
class LeetCodeSyncService:
    ...
    async def _sync_user(self, user: User, db: Session):
        """
        ...
        try:
            ...
            submissions = await client.get_recent_ac_submissions(
            ...
            for submission in submissions:
                try:
                    ...
                    existing = db.query(SolvedTask).filter(
                        ...
                        SolvedTask.title == title,
                        SolvedTask.date == submission_date
                    ).first()
...
_sync_service: LeetCodeSyncService = None
...
def get_sync_service() -> LeetCodeSyncService:
    ...
    if _sync_service is None:
        _sync_service = LeetCodeSyncService(sync_interval=settings.leetcode_sync_interval)
