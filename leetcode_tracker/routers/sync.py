...
def get_difficulty_from_problem(problem_title: str, leetcode_username: str) -> str:
    """
...
async def set_leetcode_username(
    ...
    try:
        ...
        if not profile:
            raise HTTPException(
                status_code=404,
                detail=f"LeetCode user '{leetcode_username}' not found"
            )
...
async def get_sync_status(
    ...
    total_tasks = db.query(SolvedTask).filter(
        SolvedTask.user_id == current_user.id,
        SolvedTask.platform == "leetcode"
    ).count()
